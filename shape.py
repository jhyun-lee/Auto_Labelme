...
class Shape(object):
    ...
    def paint(self, painter):
        if self.points:
            ...
            pen.setWidth(max(1, int(round(1.2 / self.scale))))
            painter.setPen(pen)
            ...
            if self.label:## 사각 바인딩 박스 상단에 라벨 텍스트 추가부분 -----------------------
                if  self.label in Person_List or "Pose" in self.label:
                    ...
                    if self.label in Person_List:
                        font.setPointSize(max(20, int(round(20 / self.scale))))
                    else:
                        font.setPointSize(max(20, int(round(10 / self.scale))))
                        labeltext=labeltext.split('_')[-1]
                    ...
                    text_path.addText(self.points[0], font, labeltext)
    ...
    def drawVertex(self, path, i):## 점 그리는곳
        d = self.point_size / (self.scale*1.2) # 사이즈 
        ...
        if shape == self.P_SQUARE:
            path.addRect(point.x() - d / 2, point.y() - d / 2, d, d) ## 머지
        elif shape == self.P_ROUND:
            path.addEllipse(point, d / 1.5, d / 1.5)
        else:
