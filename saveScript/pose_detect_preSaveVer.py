...
_hands = mp_hands.Hands(static_image_mode=False,
      ...
      min_tracking_confidence =0.05) ## 4개중 base손목과 가까운거 2개 선정
...
def handDetect(image,xyposition,keypoint,label_json,p_prefix, Dealer):
    ...
    def RLHand_json(RLstr,IND):
        ...
        Person_Save_Hand_List[p_prefix+str(RLstr)]=[]
        Person_Save_Hand_List[p_prefix+str(RLstr)].append([Hands_Box_List[IND][0], Hands_Box_List[IND][1],Hands_Box_List[IND][2], Hands_Box_List[IND][3]])## 손 박스 저장
        Person_Save_Hand_List[p_prefix+str(RLstr)].append(Hands_List[IND])## 관절들 전부 저장
    ...
    if(checkbool==1):##손 하나만
        ...
        if str(hand)=='Left':
            ...
            if p_prefix+'L' in Person_Save_Hand_List:
                Hands_Box_List.append(Person_Save_Hand_List[p_prefix+str('L')][0])
                Hands_List.append(Person_Save_Hand_List[p_prefix+str('L')][1])
            ...
            if p_prefix+'R' in Person_Save_Hand_List:
                Hands_Box_List.append(Person_Save_Hand_List[p_prefix+str('R')][0])
                Hands_List.append(Person_Save_Hand_List[p_prefix+str('R')][1])
