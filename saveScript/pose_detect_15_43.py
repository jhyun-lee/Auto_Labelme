...
def pose_detection(image_path):
    ...
    _hands = mp_hands.Hands(max_num_hands=4) ## 4개중 base손목과 가까운거 2개 선정
    ...
    for i in range(len(boxes)):
        if confs[i] > 0.99:
            ...
            # Personprediction_Check['P1']=1

            # Personprediction_Check['P10']=1
            ...
            if p_name in Personprediction_Check:  ## 사람 여러개 잡힐때 중복되게 만들어야됨!
                
                min_x=min(min_x,Playerbox[p_name][0])
                min_y=min(min_y,Playerbox[p_name][1])
                max_x=max(max_x,Playerbox[p_name][2])
                max_y=max(max_y,Playerbox[p_name][3])
                
                
                Playerbox[p_name]=[min_x, min_y,max_x, max_y]
                ...
                Personprediction_Check.setdefault(p_name, keypoints)
                Playerbox[p_name]=[min_x, min_y,max_x, max_y]
    ...
    for i in range(len(Playerbox)):
        ...
        if results.multi_hand_landmarks:
            ...
            if(checkbool):
                ...
                for i in range(0,5):
                    label_json['shapes'].append({
                        "label": f'{p_prefix}_'+f'{hand[0]}'+'H_skel',
                        "points": [
